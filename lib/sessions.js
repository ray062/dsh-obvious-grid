 ...
 *
 *   running        <- turn/start, step/start, compaction/start
 *                                                      (a turn is executing: steps,
 *                                                       tool calls, compaction)
 *   waiting        <- approval/asked                   (blocked on the human)
 *   idle           <- turn/end                         (no open turn)
  ...
  return {
    ...
    replayEvents(session) {
      ...
      for (const event of events) {
        ...
        switch (event.type) {
          ...
          case "user/message":
            ...
            break
          case "turn/start":
            rec.openTurn = { turn: typeof event?.data?.turn === "number" ? event.data.turn : rec.turns + 1, startedAt: now }
            if (rec.state !== STATE_WAITING) bump(rec, STATE_RUNNING, now)
            break
          case "step/start":
            rec.apiCalls += 1
          ...
          case "assistant/message": {
            ...
            if (!rec.openTurn && rec.state === STATE_RUNNING) bump(rec, STATE_IDLE, now)
            break
          }
          case "approval/asked":
          ...
          case "compaction/start":
            rec.compacting = true
            rec.compactions += 1
            if (rec.state === STATE_IDLE) bump(rec, STATE_RUNNING, now)
            break
          case "compaction/end":
            rec.compacting = false
            if (rec.state === STATE_RUNNING && !rec.openTurn) bump(rec, STATE_IDLE, now)
            break
          case "approval/decided":
            rec.pendingApprovals = Math.max(0, rec.pendingApprovals - 1)
            if (rec.pendingApprovals === 0 && rec.state === STATE_WAITING) bump(rec, rec.openTurn ? STATE_RUNNING : STATE_IDLE, now)
            break
          default:
            break
        }
      ...
      const lastTime = Array.isArray(events) && events.length
        ...
        : Date.now()
      if (rec.state === STATE_RUNNING || rec.openTurn || rec.compacting) {
        rec.openTurn = null
        rec.compacting = false
    ...
    snapshot() {
      ...
      for (const rec of records.values()) {
        ...
        instances.push({
          ...
          turns: rec.turns,
          steps: rec.steps,
          openTurn: !!rec.openTurn,
          compacting: rec.compacting,
